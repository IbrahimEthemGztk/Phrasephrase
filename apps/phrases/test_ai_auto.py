    ...
    def test_phrase_the_user_already_knows_is_rejected_ignoring_case_and_punctuation(self):
        for known in (PHRASE, 'actions speak LOUDER than words!', '  Actions  speak louder, than words.  '):
            with self.subTest(known), self.assertRaises(ai.AIOutputError):
                ai.parse_auto_reply(auto_json(), [known], EN)

    def test_other_known_phrases_do_not_block(self):
        ai.parse_auto_reply(auto_json(), ['Under the weather', 'Once in a blue moon'], EN)

    def test_prompt_example_phrases_are_rejected_even_when_the_user_knows_nothing(self):
        for example in EN.example_phrases:
            with self.subTest(example), self.assertRaises(ai.AIOutputError):
                ai.parse_auto_reply(auto_json(phrase=example), [], EN)
        with self.assertRaises(ai.AIOutputError):
            ai.parse_auto_reply(auto_json(phrase='break a leg.'), [], EN)

    def test_length_limits(self):
        eight = ' '.join(['word'] * ai.AUTO_MAX_WORDS)
        ai.parse_auto_reply(auto_json(phrase=eight), [], EN)   # sınırda kabul edilir
        with self.assertRaises(ai.AIOutputError):
            ai.parse_auto_reply(auto_json(phrase=eight + ' extra'), [], EN)
        with self.assertRaises(ai.AIOutputError):
            ai.parse_auto_reply(auto_json(phrase='a' * (ai.MAX_PHRASE_LENGTH + 1)), [], EN)
...
class BuildAutoInputTests(SimpleTestCase):
    ...
    def test_known_phrases_are_deduplicated_and_capped(self):
        ...
        text = ai.build_auto_input('random', ['Break a leg', *many], 'yemek', EN)
        self.assertEqual(text.count('- Break a leg'), 1)
        listed = [line for line in text.splitlines() if line.startswith('- ')]
        self.assertEqual(len(listed), ai.AUTO_AVOID_IN_PROMPT + len(EN.example_phrases))

    def test_each_known_phrase_is_cut_to_the_phrase_length_limit(self):
        text = ai.build_auto_input('random', ['x' * 100_000], 'yemek', EN)
    ...
    def test_every_category_has_a_prompt_description(self):
        for key, label in ai.CATEGORY_CHOICES:
            self.assertIn('İstenen tür:', ai.build_auto_input(key, [], 'zaman', EN))
...
class CreateCallTests(SimpleTestCase):
    def test_auto_call_uses_the_auto_prompt_schema_and_a_random_theme(self):
        with mock.patch('apps.phrases.ai._call_model') as call_model:
            ai._create_auto_interaction('proverb', ['Break a leg'], EN)
        system_instruction, input_text, schema = call_model.call_args.args
        self.assertEqual(system_instruction, ai.auto_system_instruction(EN))
    ...
    def test_given_phrase_call_still_uses_the_original_prompt_and_schema(self):
        with mock.patch('apps.phrases.ai._call_model') as call_model:
            ai._create_interaction(['Break', 'a', 'leg'], EN)
        system_instruction, input_text, schema = call_model.call_args.args
        self.assertEqual(system_instruction, ai.system_instruction(EN))
...
class GenerateAutoPhraseTests(SimpleTestCase):
    def generate(self, side_effect, category='random', language_code='en', avoid=()):
        with mock.patch('apps.phrases.ai._create_auto_interaction', side_effect=side_effect) as create:
            try:
                return ai.generate_auto_phrase(category, language_code, avoid), create
    ...
    def test_success_returns_the_ai_chosen_phrase_with_usage(self):
        ...
        create.assert_called_once_with('idiom', ['Under the weather'], EN)
    ...
    def test_unknown_language_is_rejected_before_calling_the_model(self):
        error, create = self.generate([model_reply(auto_json())], language_code='xx')
        create.assert_not_called()
        self.assertEqual(error.reason, 'invalid_input')
...
class AutoViewTests(TestCase):
    ...
    def test_success_shows_a_preview_of_the_ai_chosen_phrase_without_saving(self):
        with self.generate_patch(return_value=fake_auto()) as generate:
            response = self.client.post(self.url, {'target_language': 'en', 'category': 'idiom'})
        generate.assert_called_once_with('idiom', 'en', [])
        self.assertEqual(response.status_code, 200)
    ...
    def test_preview_regenerates_a_different_phrase_and_keeps_the_category(self):
        with self.generate_patch(return_value=fake_auto()):
            response = self.client.post(self.url, {'target_language': 'en', 'category': 'proverb'})
    ...
    def test_known_phrases_and_the_shown_one_are_passed_as_phrases_to_avoid(self):
        ...
        with self.generate_patch(return_value=fake_auto()) as generate:
            self.client.post(
                self.url, {'target_language': 'en', 'category': 'idiom', 'original_phrase': '  Shown   phrase '},
            )
        (category, language_code, avoid), _ = generate.call_args
        self.assertEqual((category, language_code), ('idiom', 'en'))
    ...
    def test_an_oversized_shown_phrase_is_cut_before_it_is_used_as_an_avoid_entry(self):
        with self.generate_patch(return_value=fake_auto()) as generate:
            self.client.post(
                self.url, {'target_language': 'en', 'category': 'idiom', 'original_phrase': 'word ' * 200_000},
            )
        (_, _, avoid), _ = generate.call_args
    ...
    def test_invalid_category_never_reaches_the_model(self):
        with self.generate_patch() as generate:
            response = self.client.post(self.url, {'target_language': 'en', 'category': 'hack'})
    ...
    def test_ai_error_is_shown_and_does_not_use_quota(self):
        with self.generate_patch(side_effect=ai.AIError('AI servisi şu an çok yoğun.', 'rate_limit')):
            response = self.client.post(self.url, {'target_language': 'en', 'category': 'random'})
    ...
    def test_anonymous_users_are_redirected_to_login(self):
        ...
        with self.generate_patch() as generate:
            self.assertEqual(
                anonymous.post(self.url, {'target_language': 'en', 'category': 'random'}).status_code, 302,
            )
            generate.assert_not_called()
